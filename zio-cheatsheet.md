...
| **Resoure Handling**                                         	|               	|                                                                                                                                                                                                                                                                      	|
|          `bracket` e f2 f3<br>`bracketExit` e f2 f3          	|               	| Let use a resource acquired and release it with guarantee even if interrupted (before/after acquire).<br>e: acquire the resource<br>f2: release the resource<br>f3: use the resource<br>In the case of `bracketExit` the release function give the whole `Exit` type 	|
|                `bracket` e<br>`bracketExit e`                	|               	| Curried version. f1 is the aquire function. <br>Produces a type that can be `apply` on a release function which produces a type <br>that can in turn be `apply` on a use function.                                                                                   	|
